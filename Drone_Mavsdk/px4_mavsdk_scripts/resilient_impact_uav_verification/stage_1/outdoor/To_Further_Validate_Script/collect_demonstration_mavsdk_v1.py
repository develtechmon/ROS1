...
CHANGES v6:
  ...
  - FIX: offboard.start() now streams setpoints at 20Hz in background task before
           attempting mode switch, with 3 retries — eliminates TIMEOUT on HITL serial.
           Single priming setpoint was expiring before PX4 processed the mode switch.
"""
...
async def run_episode(drone, buf, max_steps, episode_num):
    ...
    try:
        ...
        # ── Reliable offboard entry for HITL serial ──
        # Problem: a single priming setpoint can expire before PX4 processes
        # the mode switch over a slow serial link, causing TIMEOUT on start().
        # Fix: stream setpoints in a background task at 20Hz so PX4 always
        # has a fresh setpoint available when it processes the mode switch.
        # The task is cancelled immediately once offboard mode is confirmed.
        offboard_entry_ok = False

        async def stream_prime_setpoint():
            """Stream hold-position setpoints at 20Hz until cancelled."""
            while True:
                try:
        ...
        prime_task = asyncio.ensure_future(stream_prime_setpoint())
        await asyncio.sleep(0.2)  # let 4 setpoints through before attempting start

        for attempt in range(3):  # retry up to 3 times
            try:
                await drone.offboard.start()
                offboard_entry_ok = True
                break
            except OffboardError as e:
                print(f"Offboard attempt {attempt+1}/3 failed: {e}", end=" ", flush=True)
                await asyncio.sleep(0.5)  # brief pause before retry

        prime_task.cancel()  # stop streaming priming setpoints
        try:
            await prime_task
        except asyncio.CancelledError:
            pass

        if not offboard_entry_ok:
            print(f"Offboard failed after 3 attempts — skipping episode")
            try:
                await drone.action.disarm()
            except Exception:
                pass  # PX4 auto-disarms on ground
            await asyncio.sleep(3.0)
            return [], [], 0.0, False
