...
FLIP_ALTITUDE     = 20.0
BACKWARD_ALTITUDE = 40.0   # Change 1: increased from 30.0
...
async def flip(drone, direction, intensity='hard'):
    ...
    # Climb to correct altitude for this direction
    flip_alt = BACKWARD_ALTITUDE if direction == 'backward' else FLIP_ALTITUDE
    alt      = await get_altitude(drone)
    if alt < flip_alt - 2.0:
        print(f"  Climbing to {flip_alt}m...")
        current_target_z = -flip_alt
        keepalive_paused  = False
        await drone.offboard.set_position_ned(
            PositionNedYaw(0.0, 0.0, -flip_alt, 0.0)
        )
        await wait_altitude(drone, flip_alt)
    ...
    if direction == 'backward':
        # Change 2: higher spin thrust for backward
        # Change 3: shorter spin time — fire thrust before full 360°
        spin_thrust = 0.85
        spin_time   = 0.18   # past inverted (~310°), not waiting full 360°

        print(f"  Phase 2: Timed spin {spin_time:.3f}s "
              f"thrust={spin_thrust}...")
        await drone.offboard.set_attitude_rate(
            AttitudeRate(roll_deg_s=0.0, pitch_deg_s=pitch_rate,
                         yaw_deg_s=0.0,  thrust_value=spin_thrust)
        )
        await asyncio.sleep(spin_time)

        # Phase 3a: Full thrust burst
        print("  Phase 3a: Full thrust burst (0.8s)...")
        await drone.offboard.set_attitude_rate(
            AttitudeRate(roll_deg_s=0.0, pitch_deg_s=0.0,
                         yaw_deg_s=0.0,  thrust_value=1.0)
        )
        await asyncio.sleep(0.8)

        # Phase 3b: Zero rates stabilize
        print("  Phase 3b: Stabilize (0.5s)...")
        await drone.offboard.set_attitude_rate(
            AttitudeRate(roll_deg_s=0.0, pitch_deg_s=0.0,
                         yaw_deg_s=0.0,  thrust_value=0.7)
        )
        await asyncio.sleep(0.5)

    else:
        # All other directions: detection-based
        ...
        while asyncio.get_event_loop().time() - start < 3.0:
            if is_pitch:
                ...
                print(f"  qw={w:.3f}  qy={y:.3f}  signed_y={signed_y:.3f}")
                if signed_y > 0.707:
                    print("  ✓ Past 90°!")
                    break
            else:
...
async def main():
    ...
    for i, direction in enumerate(FLIP_SEQUENCE):
        ...
        flip_alt = BACKWARD_ALTITUDE if direction == 'backward' else FLIP_ALTITUDE
        await wait_stable(drone, target_z=flip_alt)

        if i < len(FLIP_SEQUENCE) - 1:
            next_dir = FLIP_SEQUENCE[i + 1]
            next_alt = BACKWARD_ALTITUDE if next_dir == 'backward' else FLIP_ALTITUDE
            alt      = await get_altitude(drone)
            if alt < next_alt - 3.0:
                print(f"  Climbing to {next_alt}m for next flip...")
                current_target_z = -next_alt
                keepalive_paused  = False
                await drone.offboard.set_position_ned(
                    PositionNedYaw(0.0, 0.0, current_target_z, 0.0)
                )
                await wait_altitude(drone, next_alt)
            await asyncio.sleep(3.0)
